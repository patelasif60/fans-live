...
@section('page-scripts')
	<script src="{{ asset('js/backend/pages/products/edit.js') }}"></script>
@endsection

@section('content')
	<div class="block">
		<form class="edit-product-form repeater" id="edit_product_form"
			  action="{{ route('backend.product.update', ['club' => app()->request->route('club'), 'product' => $product]) }}"
			...
			<div class="block-content">
				<div class="block block-bordered">
					...
					<div class="block-content tab-content">
		                                        ...
		                                        <div class="logo-input flex-grow-1">
		                                            ...
		                                            <div class="input-group">
		                                                <div class="custom-file">
		                                                    <div>
		                                                    	<input type="hidden" value="{{ isset($product->image_file_name) ? $product->image_file_name : '' }}" id="image_file_name" name="image_file_name">
		                                                        <input type="file" class="form-control custom-file-input" id="logo" name="logo" data-toggle="custom-file-input" accept="image/png">
		                                                        <label class="form-control custom-file-label text-truncate pr-100" for="logo">{{ isset($product->image_file_name) ? $product->image_file_name : 'Choose file' }}</label>
		                                                    </div>
		                                                </div>
		                                            </div>
		                                        </div>
		                                        <div id="logo_preview_div" class="ml-3 {{ $product->image ? '' : 'd-md-none' }}">
			                                        <div id="logo_preview_container">
			                                            <div class="logo_preview_container">
			                                                <img src="{{ $product->image }}" id="logo_preview" alt="Product logo">
			                                            </div>
			                                        </div>
			                                    </div>
		                                    </div>
											...
											@foreach($productStatus as $productStatusKey => $status)
												<div class="custom-control custom-radio custom-control-inline mb-5">
													<input class="custom-control-input" type="radio" name="status"
														   id="status_{{$productStatusKey}}"
														   value="{{$status}}" {{ $product->status == $status ? 'checked': '' }}>
													<label class="custom-control-label"
														   for="status_{{$productStatusKey}}">{{$status}}</label>
												</div>
											@endforeach()
										</div>
									</div>
										...
										<div>
											<div class="custom-control custom-checkbox custom-control-inline mb-5">
												<input type="checkbox" class="custom-control-input"
													   ...
													   @if($product->is_restricted_to_over_age == 1) checked @endif >
						...
						<div class="tab-pane" id="btabs-pricing" role="tabpanel">
								...
								<div class="col-xl-6">
									<div class="form-group{{ $errors->has('price') ? ' is-invalid' : '' }}">
										...
										<div class="input-group">
											...
											<input type="text" class="form-control" name="price" value="{{ formatNumber($product->price) }}">
										</div>
								...
								<div class="col-xl-6">
									<div class="form-group">
										...
										<input type="text" class="form-control" name="rewards_percentage_override"
											   value="{{ $product->rewards_percentage_override }}">
									</div>
								</div>
							</div>
							...
							<div class="row" class="js-custom-option-div"
								 id="edit_custom_option">
								@foreach($productOptions as $productOptionKey => $productOption)
									<div class="col-xl-12 add-home-team" id='{{$productOptionKey}}'>
										<div class="block block-bordered block-default block-rounded js-home-main-div">
											<div class="block-header block-header-default">
												<div></div>
												<div class="block-options">
													<button type="button" class="btn-block-option js-custom-option-delete"
															title="" data-original-title="Delete"><i class="fal fa-trash"></i>
													</button>
												</div>
											</div>
											<div class="block-content">
												<div class="row">
													<div class="col-xl-4">
														<div class="form-group">
															<label for="name{{$productOptionKey}}"
																   class="required">Name:</label>
															<input type="text" class="form-control custom-option-name-cls"
																   id="name{{$productOptionKey}}"
																   name="name['{{$productOptionKey}}']"
																   value="{{$productOption['name'] }}">
														</div>
													</div>
													<div class="col-xl-4">
														<div class="form-group">
															<label for="additional_cost{{$productOptionKey}}">Additional base
																cost:</label>
															<div class="input-group">
																<div class="input-group-text">
																	<i class="font-size-sm font-w600 text-uppercase text-muted">{{ $currencySymbol[$club->currency] }}</i>
																...
																<input type="text" min="0"
																   class="form-control custom-option-number-cls"
																   id="additional_cost{{$productOptionKey}}"
																   name="additional_cost['{{$productOptionKey}}']"
																   value="{{formatNumber ($productOption['additional_cost'])}}">
															</div>
														</div>
													</div>
												</div>
							...
							<div class="row js-line-ups-detail-div">
								<div class="col-xl-4">
									<div class="form-group">
										<button type="button" class="btn btn-block btn-noborder btn-primary js-added-home js-custom-option-btn">Add option</button>
									</div>
								</div>
							</div>
													...
													<div>
														<div class="custom-control custom-checkbox custom-control-inline mb-5">
															<input type="checkbox" class="custom-control-input"
																   ...
																   @if(in_array($category['id'], $productCategory)) checked @endif>
															<label class="custom-control-label"
											...
											<div>
												<div class="custom-control custom-checkbox custom-control-inline mb-5">
													<input disabled checked type="checkbox" class="custom-control-input"
														   id="collectionPoints_{{$collectionPointsKey}}"
														   name="collectionPoints[{{$collectionPointsKey}}]">
													<label class="custom-control-label"
														   for="collectionPoints_{{$collectionPointsKey}}">{{$collectionPointsValue}}</label>
												</div>
											</div>
										@endforeach
									</div>
				...
				<div class="row">
					<div class="col-xl-12">
						<div class="form-group">
							<button type="submit" id="btn_product_submit"
									class="btn btn-hero btn-noborder btn-primary min-width-125">Update
							</button>
							<a href="{{ route('backend.product.index', ['club' => app()->request->route('club')]) }}"
