...
@section('page-scripts')
    <script src="{{ asset('js/backend/pages/collectionpoints/create.js') }}"></script>
@endsection

@section('content')
	<div class="block">
		<div class="block-header block-header-default">
            <h3 class="block-title">Add collection point</h3>
        ...
        <div class="block-content block-content-full">
        	<div class="row">
                <div class="col-xl-12">
               		<form class="create-collection-point-form" action="{{ route('backend.collectionpoint.store', ['club' => app()->request->route('club')]) }}" method="post" enctype="multipart/form-data">
               			@csrf
	               		<div class="row">
	               			<div class="col-xl-6">
                            ...
                            <div class="col-xl-6 js-blocks {{ ($clubDetail->stadium && $clubDetail->stadium->is_using_allocated_seating == 0) ? 'd-none' : '' }}">
                                <div class="form-group">
                                    ...
                                    <select class="custom-select form-control answer-group" id="blocks" name="blocks[]" multiple size="5">
                                        @foreach($stadiumBlockList as $blockKey => $block)
                                            <option value="{{ $blockKey }}">{{ $block }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
