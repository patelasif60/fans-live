...
window.line_intersects = function(p0, p1, p2, p3) {
...
window.clearCanvas = function(status){
    if (document.getElementById('coordinates').value != '' && status)
    { // on edit get DB data and set it.
        perimeter = JSON.parse(document.getElementById('coordinates').value);
        complete = true;
        ctx = undefined;
        start(true);
    }
...
window.check_intersect = function(x,y){
...
window.start = function(with_draw) {
    var img = new Image();
