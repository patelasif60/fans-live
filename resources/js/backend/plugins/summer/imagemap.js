...
(function() {
    ...
    window.app = (function() {
        ...
        var areasIO = {
            ...
            selectedElementToJSON : function() {
                let obj;

                utils.foreach(state.areas, function(x) {
                    if(!x._groupEl.attributes.hasOwnProperty('pointer-events')) {
                        obj = x.toJSON();
                    }
                });
        ...
        var localStorageWrapper = (function() {
            ...
            return {
                save : function() {
                    ...
                    var posData = areasIO.selectedElementToJSON();
                    window.localStorage.setItem(KEY_NAME, result);
                    $("#pos_data").val(posData);
                    swal({title: 'Saved',
                          text: "Click on ok to continue"}).
                    then(()=> {
                       var all = utils.id('nav').getElementsByClassName('js-li');
    ...
    function Area(type, coords, attributes) {
        ...
        if(typeof attributes !== 'undefined' && attributes.hasOwnProperty("ID") && $('#stadium_block_id').val() !== attributes['ID']) {
            $(this._groupEl).attr('pointer-events', 'none');
            $(this._groupEl).attr('opacity', '0.4');
        }
    }
    ...
    Rectangle.prototype.toHTMLMapElementString = function() {
        ...
        return '<area shape="rect" coords="' // TODO: use template engine
            ...
            + (this._attributes.id ? ' data-id="' + this._attributes.id + '"' : '')
            + ' />';
    };
    ...
    var buttons = (function() {
        ...
        function onRemoveElementButtonClick(e) {
            // Remove current element
            // if (confirm('Remove current element?')) {
            //     buttons.removeCurrentElement(e);
            // }
            swal({
            title: 'Are you sure?',
        ...
        remove.addEventListener('click', onRemoveElementButtonClick, false);
        ...
        return {
            ...
            removeCurrentElement : function() {
                if($('#image g[pointer-events!=none]').length > 0) {
                    let elementIndex = -1;
                    $.each(app.domElements.svg.childNodes, function(index, g) {
                        if(!g.hasOwnProperty('pointer-events')) {
                            elementIndex = index;
                        }
                    });
                    if(elementIndex !== -1) {
                        app.domElements.svg.removeChild(app.domElements.svg.childNodes[elementIndex]);
                        app.setMode(null)
                            .setDefaultClass()
                            .setShape(null)
                            .hidePreview();
